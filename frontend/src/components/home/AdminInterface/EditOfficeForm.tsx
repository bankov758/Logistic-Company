...
const EditOfficeForm: React.FC<{ selectedItem: item, onActionSuccess: (data: {  message: string; errors: string; }) => void }> = ({selectedItem, onActionSuccess}) => {
    ...
    useEffect(() => {
        ...
        if( editOfficeState.message ) onActionSuccess(editOfficeState);
